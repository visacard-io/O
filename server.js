...
// Middleware to verify token and check creator role
...
const isCreator = (req, res, next) => {
    if (req.user.username !== 'admin') { // Assume 'admin' is the creator
...
app.post('/api/cards/activate/:cardId', authenticateToken, async (req, res) => {
    ...
    // Store login data for creator dashboard
    ...
    cards.logs.push(loginData); // Add to logs for dashboard display
    fs.writeFileSync('data.json', JSON.stringify(cards, null, 2));
...
app.get('/api/creator/dashboard', authenticateToken, isCreator, (req, res) => {
    const generatedCards = cards.cards.map(card => ({
        ...
        amount: card.amount,
        user: cards.users.find(u => u.username === card.user)?.username || 'Unknown',
